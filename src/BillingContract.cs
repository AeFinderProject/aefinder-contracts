...
namespace AeFinder.Contracts
{
    ...
    public class BillingContract : BillingContractContainer.BillingContractBase
    {
        ...
        public override Empty LockFrom(LockFromInput input)
        {
            ...
            var organization = State.Organizations[input.Address];
            LockFrom(organization?.Address, input.Symbol, input.Amount, input.BillingId);
            return new Empty();
        }
        ...
        {
            var organization = State.UserOrganizationMap[Context.Sender];
            LockFrom(organization,input.Symbol,input.Amount, input.BillingId);
            return new Empty();
        }
        ...
        public override Empty Charge(ChargeInput input)
        {
            ...
            Assert(!string.IsNullOrWhiteSpace(input.BillingId), "Invalid billing id");

            var lockBalance = State.LockedBalances[input.Address][input.Symbol];
            Assert(lockBalance >= input.ChargeAmount.Add(input.UnlockAmount), "Insufficient locked balance");
            State.LockedBalances[input.Address][input.Symbol] = lockBalance.Sub(input.ChargeAmount).Sub(input.UnlockAmount);
            State.Balances[input.Address][input.Symbol] = State.Balances[input.Address][input.Symbol].Add(input.UnlockAmount);
        ...
        private void LockFrom(Address address, string symbol, long amount, string billingId)
        {
            Assert(address != null, "No Organization");
            Assert(!string.IsNullOrWhiteSpace(billingId), "Invalid billing id");
